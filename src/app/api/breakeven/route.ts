...
export async function GET(request: NextRequest) {
  ...
  const gpuId = params.get("gpuId");

  type ModelRow = {
    id: string; model_name: string; provider: string;
    input_price_per_mtok: number; output_price_per_mtok: number; quality_tier: string;
  };
  type GpuRow = { gpu_model: string; price_per_hour: number; region: string; reliability_score: number };

  let bestModel: ModelRow | undefined;
  let bestGpu: GpuRow | undefined;

  if (modelId) {
    bestModel = db.prepare(`
